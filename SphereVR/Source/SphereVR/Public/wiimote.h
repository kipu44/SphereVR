...
struct wiimote_state_event {
	uint32		  time_ms;	// system timestamp in milliseconds
	wiimote_state state;
	};
...
class wiimote : public wiimote_state
	...
	private: // methods
		...
		void OnReadData				(uint32 bytes_read);
...
volatile int	 MotionPlusDetectCount;		  // waiting for the result
		...
		//  detection)
		uint32			  NextStatusTime;
		uint32			  NextMPlusDetectTime;// gap between motion plus detections
		uint32			  MPlusDetectCount;	  // # of detection tries in quick succesion
		...
		volatile uint32	 AsyncRumbleTimeout;
		...
		struct recording
			...
			volatile uint32	StartTimeMS;
			volatile uint32	EndTimeMS;		// can be UNTIL_STOP
